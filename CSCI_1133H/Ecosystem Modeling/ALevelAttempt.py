...
class Organism:
    ...
    def reproduce(self, world):
        cell = world.get_grid()[self.get_location()[1]][self.get_location()[0]]
        cell.add_immigrant(type(self)(self.get_location()))
...
class Animal(Organism):
    def __init__(self, energy, lifeSpan, location, upkeep, foods, gestation_time):
        super().__init__(energy, lifeSpan, location, upkeep)
        ...
        self.__days_until_fertile = gestation_time
        self.__gestation_time = gestation_time
        self.__thirst = 0
    ...
    def live_one_day(self, world):
        ...
        self.hunt(world)
        self.migrate(world)
        self.__days_until_fertile -= 1
        
    def reproduce(self, world):
        if self.__days_until_fertile <= 0:
            Organism.reproduce(world)
            self.__days_until_fertile = self.__gestation_time
    
    def found_water(self, world):#checking to see if org finds water (5 tries)
        cell = world.get_grid_at_location(self, self.__location[0], self.__location[1])
        max_attempts_to_find_water = 5
        for i in range(max_attempts_to_find_water):
            if random.random <= cell.getWater():
                return True
        return False
    
    def drink(self, world): #If org found water, quench thirst
        if self.found_water(self, world): #I'm assuming you want me to call found_water
            self.__thirst = 0
        else:
            self.__thirst += 0.25
    
    def is_alive(self):
        return Organism.is_alive(self) and self.__thirst < 1
...
class Wildebeest(Animal):
    def __init__(self,location, gestation_time = 15):
        energy = 7500
        lifespan = 30
        upkeep = 100
        foods = {Grass:0.05} 
        super().__init__(energy, lifespan, location, upkeep, foods, gestation_time)
        
class Lion(Animal):
    def __init__(self, location, gestation_time = 10):
        energy = 5000
        lifespan = 25
        upkeep = 50
    ...
    def migrate(self, world):
        if random.random() < 1/3.0 and self.get_fill() < 1:
            world.move_organism(self)
